...
const createClubValidation = [
  body('name').trim().isLength({ min: 2, max: 100 }).withMessage('Tên quán phải từ 2 đến 100 ký tự'),
  ...
  body('pricePerHour').isFloat({ min: 0 }).withMessage('Giá thuê phải là số dương'),
  body('location.coordinates').isArray({ min: 2, max: 2 }).withMessage('Tọa độ phải có 2 giá trị [longitude, latitude]'),
  body('location.coordinates.*').isNumeric().withMessage('Tọa độ phải là số')
];

const updateClubValidation = [
  body('name').optional().trim().isLength({ min: 2, max: 100 }).withMessage('Tên quán phải từ 2 đến 100 ký tự'),
  body('phone').optional().matches(/^[0-9]{10,11}$/).withMessage('Số điện thoại không hợp lệ'),
  body('email').optional().isEmail().withMessage('Email không hợp lệ'),
  body('pricePerHour').optional().isFloat({ min: 0 }).withMessage('Giá thuê phải là số dương')
];

const rateClubValidation = [
  body('rating').isFloat({ min: 1, max: 5 }).withMessage('Đánh giá phải từ 1 đến 5')
];
